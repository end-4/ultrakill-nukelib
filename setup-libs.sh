...
TARGET_DIR="$SCRIPT_DIR/libs/Managed"
...
cp -v "$R2MODMAN_PROFILE_PATH"/BepInEx/core/BepInEx.dll "$TARGET_DIR/" 2>/dev/null || print_error "Could not copy BepInEx.dll"
cp -v "$R2MODMAN_PROFILE_PATH"/BepInEx/core/0Harmony.dll "$TARGET_DIR/" 2>/dev/null || print_error "Could not copy 0Harmony.dll"

print_info $'\nSetup finished. If any errors occurred, please specify your paths manually.'
print_info "You can now build the mod using make-package.ps1 (or a corresponding shell script)."
